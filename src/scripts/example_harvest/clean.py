...
def fix_tones(word:str):
	...
	mapping = str.maketrans("".join(floating),"".join(practical))
	ret_word = word.translate(mapping)
	return ret_word
			...
			for filename in prog(files_to_clean,"Cleaning Files",colour="green"):
				...
				with open(filename,"r",encoding="utf-8") as readFile, open(os.path.join(cleaned_files_dir,f"cleaned_{os.path.basename(filename)}"),"w",encoding="utf-8") as writeFile:
					...
					if exp_mode:
						...
						sents = fileStr.splitlines()
						for sent in sents:
							words = [fix_tones(wrd.strip(" .")) for wrd in sent.split()]
							writeFile.write("\n".join(words))
							writeFile.write("\n\n")
						readFile.close()
						writeFile.close()
